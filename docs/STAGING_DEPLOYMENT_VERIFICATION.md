...
> **Region**: `us-central1`  
> **Service URL**: `https://gemini-journal-staging-s7hw7hui2q-uc.a.run.app`  
...
Each core subsystem was verified on the live staging Cloud Run environment (`https://gemini-journal-staging-s7hw7hui2q-uc.a.run.app`):
...
| **Gemini AI** | Server proxy (`/api/gemini/*`), model fallback ladder (`gemini-2.5-flash` → `2.5-pro` → `2.0-flash`), IP rate limiter | **PASS** | Server endpoint active and responding cleanly |
...
- **Verification**:
  - `npm run smoke` returned `🎉 ALL SMOKE TESTS PASSED CLEANLY!` against `https://gemini-journal-staging-s7hw7hui2q-uc.a.run.app`.

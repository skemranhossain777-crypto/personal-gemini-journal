      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-slate-900 border border-slate-800 space-y-3">
          <div className="flex items-center gap-2 text-rose-400">
            ...
            <h3 className="text-sm font-semibold">Destructive Data Actions</h3>
          </div>
          <div className="space-y-2">
            <button
              onClick={() => setActiveModal('memories')}
              className="w-full py-2 rounded-xl bg-slate-950 border border-slate-800 hover:border-rose-500/40 text-xs text-rose-300 hover:text-rose-200 flex items-center justify-between px-3 transition"
            >
              <span>Delete AI Memories</span>
            ...
            <button
              ...
              className="w-full py-2 rounded-xl bg-slate-950 border border-slate-800 hover:border-rose-500/40 text-xs text-rose-300 hover:text-rose-200 flex items-center justify-between px-3 transition"
            >
              <span>Delete Journal Data & Media</span>
              <Trash2 className="w-3.5 h-3.5" />
            </button>
            ...
            <button
              ...
              aria-label="Wipe Account Data Permanently"
              className="w-full py-2 rounded-xl bg-rose-950/40 border border-rose-800/60 hover:bg-rose-900/60 text-xs text-rose-200 font-semibold flex items-center justify-between px-3 transition min-h-[44px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-rose-400"
            >
              <span>Wipe Account Data Permanently</span>
      ...
      <Dialog
        ...
        title={
          activeModal === 'memories'
            ? 'Confirm Delete AI Memories'
            : activeModal === 'journal'
            ? 'Confirm Delete Journal Data'
            : 'Permanently Delete Account Data'
        }
        description={
          activeModal === 'memories'
            ? 'This will permanently delete all extracted personal memories. This action cannot be undone.'
            : activeModal === 'journal'
            ? 'This will permanently delete all journal entries and attached images/audio files.'
            : 'This will wipe ALL entries, memories, goals, habits, and preferences.'
        }
        icon={<AlertTriangle className="h-5 w-5 text-rose-400" />}
        iconClassName="bg-rose-950/60 border border-rose-800/40"
        footer={
          <>
            <Button variant="subtle" size="sm" onClick={() => setActiveModal(null)}>
              Cancel
            </Button>
            <Button
              variant="danger"
              ...
              disabled={activeModal === 'account' && confirmInput.trim().toUpperCase() !== 'DELETE'}
              onClick={
                activeModal === 'memories'
                  ? handleConfirmDeleteMemories
                  : activeModal === 'journal'
      ...
      >
        <div className="space-y-4 p-6">
          <p className="text-xs leading-relaxed text-slate-300">
            {activeModal === 'memories' &&
              'All extracted candidate and saved memories will be deleted from your personal store. Ask My Life will no longer reference them.'}
            {activeModal === 'journal' &&
              'All journal entries, attachments, and reflection histories will be permanently removed. Ask My Life context will be cleared.'}
            {activeModal === 'account' &&
              'Warning: This action is irreversible. Type "DELETE" in the box below to authorize complete account data wipe.'}
          </p>

          {activeModal === 'account' && (
            <div>
              <label htmlFor="confirm-delete-account-input" className="block text-xs font-medium text-slate-300 mb-1.5">
                Confirmation Phrase
              </label>
              <input
                ...
                className="w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-200 placeholder:text-slate-500 focus:border-rose-500 focus:outline-none focus:ring-2 focus:ring-rose-500/40"
              />
            </div>
          )}
        </div>
